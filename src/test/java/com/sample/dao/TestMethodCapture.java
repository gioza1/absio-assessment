 ...
 * listener</a>, then access the method and result from test code with the static {@link #getTestMethod} and {@link
 * #getTestResult} methods.
 *
 * <p>Annotating a test class with {@code @Listeners(TestMethodCapture.class)} is the
 * suggested way to enable capturing if your test's correctness will depend on this listener being enabled.
 */
public class TestMethodCapture implements IInvokedMethodListener {
    private static final AtomicLong counter = new AtomicLong(1);
    private static ThreadLocal<Long> currentCounter = new ThreadLocal<>();
    ...
    public static long getCounter() {
        return checkNotNull(currentCounter.get(),
    ...
    public void beforeInvocation(IInvokedMethod method, ITestResult testResult) {
        currentCounter.set(counter.getAndIncrement());
